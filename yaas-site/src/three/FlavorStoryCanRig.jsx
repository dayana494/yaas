...
export default function FlavorStoryCanRig({ flavorId, rotation, spin = false, anchor = null }) {
  ...
  // How far off centre the can sits on the first screen.
  //
  // `anchor` arrives from FlavorStorySection as {mode, at}: a fraction of the
  // screen, and which part of the can that fraction refers to — 'foot' stands
  // it on the flavor name's baseline (desktop), 'centre' floats its middle
  // there (mobile, where it is centred in the gap between wordmark and name).
  // The fraction is measured off the rendered DOM because that is the only
  // thing the copy and this canvas have in common: the name lives in a scaled
  // CSS stage, the can in world units. World y runs +height/2 at the top of
  // the frame to -height/2 at the bottom, so the fraction converts straight
  // across; 'foot' then adds half the can's own height. Null (one frame,
  // before the measurement lands) means no offset.
  //
  ...
  const drop = useMemo(() => {
    if (!anchor) return 0;
    const target = viewport.height * (0.5 - anchor.at);
    return anchor.mode === 'centre' ? target : target + (height * scale) / 2;
  }, [anchor, viewport.height, height, scale]);
  ...
  useFrame((_, delta) => {
    ...
    if (spin) {
      ...
      // composing a screen around the can, so there is no line to stand it on.
      group.position.y = 0;
